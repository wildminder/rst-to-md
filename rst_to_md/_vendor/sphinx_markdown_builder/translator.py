...
PREDEFINED_ELEMENTS: Dict[str, Union[PushContext, SKIP, None]] = dict(  # pylint: disable=use-dict-literal
    ...
    # NOTE (rst-to-md vendored patch): `desc_annotation` (e.g. `property`,
    # `class`, `staticmethod`) is rendered as plain text, NOT italics, so that
    # autodoc member signatures read `property token: str` instead of
    # `*property* token: str*`. Matches the legacy `html` builder output.
    ...
    # NOTE (rst-to-md vendored patch, D1): field lists (Parameters:/Raises:/...)
    # must NOT become nested bullet lists. We pass `field_list`/`field` through
    # (no list marker) and indent the `field_body` under the bold field name,
    # matching the legacy `html` builder output.
    field_list=None,
    field=None,
    field_body=PushContext(
...
class MarkdownTranslator(SphinxTranslator):  # pylint: disable=too-many-public-methods
    ...
    def visit_reference(self, node):
        # NOTE (rst-to-md vendored patch, XREF): inside an autodoc signature
        # (desc_depth > 0) type cross-references are emitted as plain text, not
        # Markdown links. The generated anchors (#fully.qualified.name) are
        # ambiguous/broken in Markdown viewers and add no navigation value, and
        # the legacy `html` builder renders the same annotation as plain text.
        # We still push a neutral SubContext (no prefix/suffix) so the
        # @pushing_context depart stays stack-balanced; the link label text is
        # written through unchanged. Prose links elsewhere keep [text](url).
        if self.status.desc_depth > 0:
    ...
    def visit_desc(self, node):
        # NOTE (rst-to-md vendored patch): track desc nesting depth so that
        # nested member signatures (properties/methods inside a class) are
        # rendered as bold paragraphs instead of `####` headings (D3 fix).
        self._push_status(
            desc_type=node.attributes.get("desctype", ""),
            desc_depth=self.status.desc_depth + 1,
        )
    ...
    def visit_desc_signature(self, node):
        ...
        # NOTE (rst-to-md vendored patch, D3): member *properties/attributes*
        # (desc_type in {property, attribute}) are emitted as a bold paragraph
        # `**name**` instead of a heading, because a heading for a one-line
        # property reads as noise. Methods and functions keep a heading (the
        # user expects `### context(...)` for a method), matching the legacy
        # html builder.
        if self.status.desc_depth > 1 and self.status.desc_type in (
            "property",
            "attribute",
        ):
            self._push_context(
                WrappedContext("**", "**", params=SubContextParams(2, 2))
            )
            return

        # Heading levels (rst-to-md vendored patch, SECTION header):
        # the top-level autodoc object (class/function at desc_depth == 1)
        # becomes an `##` (h2) SECTION header so the document has a proper
        # `#` (page title) -> `##` (autodoc section) -> `###` (members)
        # hierarchy instead of jumping straight from h1 to h3/h4. Nested
        # members (desc_depth > 1) are `###` (h3).
        h_level = 3 if self.status.desc_depth > 1 else 2
