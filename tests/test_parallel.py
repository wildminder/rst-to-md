...
def test_sphinx_parallel_same_result_as_serial(sphinx_min_project: Path, tmp_path: Path):
    out_serial = tmp_path / "serial"
    out_par = tmp_path / "par"
    with mock.patch("html_to_markdown.convert", return_value="# x\n"):
        ...
        r_par = convert_sphinx_project(sphinx_min_project, out_par, max_workers=4, use_cache=False)
